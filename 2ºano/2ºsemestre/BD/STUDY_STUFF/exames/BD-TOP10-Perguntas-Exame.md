...
- Sistemas com poucas updates e muitas leituras
...
1. **Escalabilidade e Pooling de Conexões**: O servidor de aplicação gama e reutiliza um conjunto de conexões (connection pool) com o SGBD, permitindo servir milhares de utilizadores concorrentes a partir do browser. Numa arquitetura de 2 níveis, cada browser precisaria de abrir e manter uma ligação dedicada à BD, esgotando rapidamente os recursos de ligação do SGBD.
...
1. **Hardware**: dispositivos físicos (servidores, discos, networks)
